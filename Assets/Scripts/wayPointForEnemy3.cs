...
public class wayPointForEnemy3 : MonoBehaviour {
	...
	void Update () {
		if(wayPointsTest3.stations != null && initTargetPos == true) {
			randNum = Random.Range(0, wayPointsTest3.wayPtNum);
			targetPos = wayPointsTest3.stations[stationIndex][randNum];
			initTargetPos = false;
		}
		...
		if(ifCloseTarget(transform.position, targetPos) == true) {
			...
			if(stationIndex < wayPointsTest3.stationNum - 1) {
				stationIndex++;
				randNum = Random.Range(0, wayPointsTest3.wayPtNum);
				if(stationIndex == wayPointsTest3.stationNum - 1) {
					randNum = 0;
